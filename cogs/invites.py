...
from .database import (
    get_invite_data, save_invite_data, add_invited_user,
    remove_invited_user, find_inviter_by_user, get_all_invite_data,
    reset_invite_data
)
...
class InviteLogger(commands.Cog):
    ...
    async def on_member_join(self, member: disnake.Member):
        ...
        try:
            ...
            if used_invite and used_invite.inviter:
                ...
                add_invited_user(inviter.id, member.id, str(member))
    ...
    async def on_member_remove(self, member: disnake.Member):
        ...
        try:
            ...
            inviter_id = find_inviter_by_user(member.id)
            inviter = guild.get_member(inviter_id) if inviter_id else None
            inviter_data = None
            
            if inviter_id:
                new_count = remove_invited_user(inviter_id, member.id)
                inviter_data = {"total_invites": new_count}
    ...
    async def leaderboard_command(self, inter: disnake.ApplicationCommandInteraction):
        ...
        try:
            ...
            all_invite_data = get_all_invite_data()
            for item in all_invite_data:
                member = inter.guild.get_member(item["user_id"])
                if member:
                    leaderboard.append((member, item["total_invites"]))
